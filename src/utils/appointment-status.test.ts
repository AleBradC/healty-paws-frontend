...
// Anchor "now" so the time-window assertions are deterministic. The chosen
// instant is arbitrary — every assertion below describes the appointment
// time relative to this anchor.
...
describe('getAppointmentDisplayStatus', () => {
  ...
  it('returns "Pending" when baseStatus is null', () => {
    expect(getAppointmentDisplayStatus(null, NOW.toISOString())).toBe('Pending');
  });
  ...
  it('returns "Upcoming" near the 2h-from-now boundary', () => {
    // Just inside the "more than 2h before" check: 2h - 1 minute away.
  ...
  it('falls back to "Confirmed" once the 1h grace window has elapsed', () => {
    // 1h 1m past the appointment time — outside the start window on both
    // ends, so we drop back to the base status.
    const longGone = addMinutes(NOW, -61 - 60).toISOString();
    expect(getAppointmentDisplayStatus('Confirmed', longGone)).toBe('Confirmed');
  });

  it('accepts a Date object as datetime input', () => {
    // 1 minute in the future → inside the start window (5m before to 1h after).
    const dateInstance = addMinutes(NOW, 1);
    expect(getAppointmentDisplayStatus('Confirmed', dateInstance)).toBe('Start');
  });
});

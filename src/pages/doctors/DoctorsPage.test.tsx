...
describe("DoctorsPage", () => {
  ...
  it("renders the page heading and search input", async () => {
    ...
    await act(async () => {});
    expect(
      screen.getByRole("heading", { name: /our doctors/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText(/search by name or clinic/i),
    ).toBeInTheDocument();
  });
  ...
  it("updates search and calls useDoctors with specialization filter", async () => {
    ...
    expect(useDoctors).toHaveBeenCalledWith(
      expect.any(Number),
      expect.any(Number),
      "",
      "s1",
  ...
  it("updates search and calls useDoctors with debounced value", async () => {
    ...
    expect(useDoctors).not.toHaveBeenCalledWith(
      expect.any(Number),
      expect.any(Number),
      "Pop",
      "",
  ...
  it("clears search when clear button is clicked", async () => {
    ...
    expect(useDoctors).toHaveBeenLastCalledWith(
      expect.any(Number),
      expect.any(Number),
      "",
      "",
  ...
  it("renders DoctorCards for each doctor with specializations", async () => {
    ...
    await act(async () => {});
    expect(
      screen.getByRole("heading", { name: "Dr. Pop" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "Dr. Ionescu" }),
    ).toBeInTheDocument();
  });

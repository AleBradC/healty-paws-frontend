...
vi.mock(
  "../../../components/features/AvailabilityModal/AvailabilityModal",
  () => ({
...
describe("DoctorDashboardPage", () => {
  ...
  it("renders the tabs", async () => {
    renderPage();
    expect(
      await screen.findByRole("button", { name: /my details/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /my services/i }),
    ).toBeInTheDocument();
  });
  ...
  it("switching to appointments tab shows appointments section", async () => {
    renderPage();
    const appointmentsTab = await screen.findByRole("button", {
      name: /my appointments/i,
    });
    await userEvent.click(appointmentsTab);
    expect(
      await screen.findByText(/you have no upcoming appointments/i),
    ).toBeInTheDocument();
  });

  it("switching to availability tab shows Manage Availability button", async () => {
    renderPage();
    const availabilityTab = await screen.findByRole("button", {
      name: /my availability/i,
    });
    await userEvent.click(availabilityTab);
    expect(
      await screen.findByRole("button", { name: /manage availability/i }),
    ).toBeInTheDocument();
  });
  ...
  it("clicking Manage Availability opens the availability modal", async () => {
    ...
    expect(
      await screen.findByText(/manage your availability/i),
    ).toBeInTheDocument();
  });
  ...
  it("shows error message when doctor data cannot be loaded", async () => {
    ...
    renderPage();
    expect(
      await screen.findByText(/could not load your information/i),
    ).toBeInTheDocument();
  });
});
